...
---@return 'windows'|'linux'|'macos'|'unknown' osname
...
function VimSockets:get_socket_paths()
	...
	if self.os.name == "linux" or self.os.name == "macos" then
		local dirs = {}

		if self.os.name == "linux" then
			table.insert(dirs, vim.env.XDG_RUNTIME_DIR)
			table.insert(dirs, "/run/user/" .. uv.getuid())
		else -- macos
			table.insert(dirs, vim.env.TMPDIR)
		end
		...
		for _, dir in ipairs(dirs) do
			...
			if not handle then
				self.logger:error("VimSockets:get_socket_paths", "Could not scan directory (" .. dir .. ")")
			else
				while true do
					...
					if ftype == "socket" and name:match("^nvim%.") then
